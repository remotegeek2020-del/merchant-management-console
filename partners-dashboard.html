...
<head>
    ...
    <style>
        ...
        body { background: #f1f5f9; padding: 25px; font-family: 'DM Sans', sans-serif; }
...
<div class="modern-header">
 ...
 <div class="step-container" id="step2">
    ...
    <div style="margin-bottom:14px;">
        ...
        <input type="date" id="partnerActivationDate" class="wizard-input" style="font-family:'DM Sans',sans-serif;">
    ...
    let filtered = allPartners.filter(p => {
        ...
        p.companies.forEach(co => {
            co.ids.forEach(idObj => {
                const s = statsRaw.find(raw => String(raw.agent_id).trim() === String(idObj.string).trim());
                if (s) {
...
async function submitOnboarding() {
    ...
    await logAuditAction('Partner ID Assignment', logMsg);
        ...
        const _activationDate = document.getElementById('partnerActivationDate');
        payload.person = {
            name: selectedGHLContact.name,
            email: selectedGHLContact.email,
            phone: selectedGHLContact.phone,
            hl_id: selectedGHLContact.id,
            enrolled_at: (_activationDate && _activationDate.value) ? new Date(_activationDate.value).toISOString() : new Date().toISOString()
        };
        payload.company = {
            ...
            <div class="partner-card" onclick="openPartnerDetails('${p.id}')">
                ...
                <div style="margin-top:12px;padding-top:10px;border-top:1px solid #f1f5f9;display:flex;gap:6px;">
                    <button onclick="event.stopPropagation();openPartnerDetails('${p.id}')"
                            style="flex:1;background:#f0f7ff;border:1px solid #bfdbfe;color:#004990;border-radius:8px;padding:7px;font-size:11px;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:4px;">
                        <span class="material-icons" style="font-size:14px;">person</span> View Detail
                    </button>
                    <button onclick="event.stopPropagation();sendPortalInvite('${p.id}','${p.name.replace(/'/g, String.fromCharCode(39))}','${(p.email||'').replace(/'/g, String.fromCharCode(39))}')"
                            style="flex:1;background:${(p.portal_active && p.portal_setup) ? '#f0fdf4' : '#f0fdfa'};border:1px solid ${(p.portal_active && p.portal_setup) ? '#bbf7d0' : '#99f6e4'};color:${(p.portal_active && p.portal_setup) ? '#166534' : '#0d9488'};border-radius:8px;padding:7px;font-size:11px;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:4px;"
                            ${(p.portal_active && p.portal_setup) ? 'disabled' : ''}>
                        <span class="material-icons" style="font-size:14px;">${(p.portal_active && p.portal_setup) ? 'verified' : (p.portal_active ? 'mark_email_read' : 'send')}</span>
                        ${(p.portal_active && p.portal_setup) ? 'Portal Active' : (p.portal_active ? 'Resend Invite' : 'Invite to Portal')}
                    </button>
                </div>
            </div>`;
    }).join('');
...
window.quickAddId = async function(personId, name) {
    ...
    async function loadPartners() {
        ...
        try {
            // Fetch partners list first, stats load separately after
            ...
            partnersData_companiesRaw = partnersData.data.companies || [];
            const companiesRaw = partnersData_companiesRaw;
            statsRaw = []; // loads in background after render

            // Populate company dropdown in wizard
            const companySelect = document.getElementById('companySelect');
            if (companySelect) {
                const existingOptions = companySelect.innerHTML;
                companiesRaw.forEach(co => {
                    const opt = document.createElement('option');
                    opt.value = co.id;
                    opt.textContent = co.company_name;
            ...
            // Load stats in background AFTER partners are visible
            loadStatsBackground();
    ...
    async function loadStatsBackground() {
        try {
            const res = await fetch('/api/partners', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'get_all_stats' })
            });
            const result = await res.json();
            if (result.success && result.data.length) {
                statsRaw = result.data;
                // Rebuild allPartners with stats so filterByTrend works
                allPartners = processPartnerData({
                    persons: personsRaw, agents: agentsRaw,
                    identifiers: identifiersRaw,
                    companies: (partnersData_companiesRaw || []),
                    stats: statsRaw
                });
                console.log('[Partners] Stats loaded -', statsRaw.length, 'rows. Trend filters ready.');
            }
        } catch(e) {
            console.warn('[Partners] Stats load failed silently:', e.message);
        }
    }

    async function sendPortalInvite(personId, name, email) {
        if (!email || email === 'No email provided') {
            return Swal.fire({ icon:'warning', title:'No email', text: name + ' has no email on file.', confirmButtonColor:'#0d9488' });
        }
        const { isConfirmed } = await Swal.fire({
            title: 'Invite ' + name + '?',
            html: '<p>An email will be sent to <b>' + email + '</b> to set up their portal account.</p>',
            icon:'question', showCancelButton:true, confirmButtonColor:'#0d9488', confirmButtonText:'Send Invite'
        });
        ...
        try {
            ...
            const result = await res.json();
            if (result.success) {
                Swal.fire({ icon:'success', title:'Invite Sent!',
                    html:'<p><b>'+name+'</b> will receive setup email at <b>'+email+'</b>.</p>' +
                         (result.invite_url ? '<div style="background:#f0fdfa;border-radius:8px;padding:10px;margin-top:10px;font-size:11px;word-break:break-all;"><b>Direct link:</b><br>'+result.invite_url+'</div>' : ''),
    ...
    window.editEnrolledAt = async function(personId, currentDate) {
        const { value: newDate } = await Swal.fire({
            ...
            html: '<input type="date" id="enrollDate" value="' + currentDate + '" style="width:100%;padding:10px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:14px;outline:none;">',
