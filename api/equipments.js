...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'updateDeployment') {
            ...
            await logAction(`Updated Deployment ${id}: Status ${payload.deployment_status}`);
        ...
        if (action === 'createDeployment') {
            ...
            const delPromises = (rels.relations || []).map(rel => 
                fetch(`${PROXY_URL}relations/${rel.id}?locationId=${LOCATION_ID}`, { method: "DELETE" })
            );
            ...
            // Execute Linking and Hardware Update in Parallel
            await Promise.all([
            ...
            await logAction(`Deployed Gear: ${payload.deployment_id}`);
            return res.status(200).json({ success: true, id: newId });
        }

        // --- ACTION: LIST DEPLOYMENTS (Optimized Hydration) ---
        if (action === 'listDeployments') {
            ...
            const hydratedRecords = await Promise.all((data.records || []).map(async (record) => {
                try {
                    ...
                    const equipmentRel = (relData.relations || []).find(r => r.firstObjectKey.includes(EQUIPMENT_SCHEMA) || r.secondObjectKey.includes(EQUIPMENT_SCHEMA) || r.firstObjectKey.includes("equipments"));
        ...
        if (action === 'createEquipment') {
            ...
            await fetch(PROXY_URL, { method: "POST", headers: { "Content-Type": "application/json", "Link-Relation": "true" }, body: JSON.stringify({ locationId: LOCATION_ID, associationId: ASSOC_ID, firstRecordId: newEq.id, secondRecordId: merchantID }) });
