...
export default async function handler(req, res) {
    ...
    try {
        if (action === 'list') {
            ...
            // This is the "Locked" structure that allows searching through all tables
            let selectString = `
                *,
            ...
            // THE LOCK: Only use !inner if we are searching by Company or Partner
            ...
            // --- 2. QUERY A: DATA FOR THE TABLE (PAGINATED) ---
            ...
            // --- 3. QUERY B: GLOBAL VOLUMES (NOT PAGINATED) ---
            // This ignores the .range() to get the total for EVERY record matching your search
            ...
            // --- 4. QUERY C: ABSOLUTE DATABASE TOTAL (FOR PORTFOLIO %) ---
            let absReq = supabase.from('merchants').select('volume_mtd');

            // --- 5. APPLY FILTERS TO DATA AND VOLUME QUERIES ---
        ...
        // ... (Update/Delete logic remains the same)
        if (action === 'update') {
