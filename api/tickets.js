    ...
    try {
        ...
        if (action === 'get_merchant_deployments') {
            ...
            const { data, error } = await supabase.from('deployments')
                .select('id, deployment_id, status, created_at, equipments:equipment_id(serial_number, terminal_type), returns(id, status)')
                .eq('merchant_id', merchant.id)
                .order('created_at', { ascending: false });
            if (error) throw error;
        ...
        if (action === 'create_linked_record') {
            ...
            if (record_type === 'deployment') {
                const { equipment_id, tid, tracking_id, target_date, purchase_type, notes } = req.body;
                if (!equipment_id) return res.status(400).json({ success: false, message: 'equipment_id required.' });

                // Resolve merchant UUID + name from text MID
                ...
                if (equip.status !== 'stocked') return res.status(400).json({
                    success: false,
                    message: `Conflict: Serial ${equip.serial_number} is not available (status: ${equip.status}).`
                });

                // Insert deployment record (deployment_id auto-generated by DB)
                ...
                // Update equipment with race condition protection
                ...
                const { deployment_internal_id, return_reason, return_date_initiated, notes } = req.body;
                ...
                // Upsert return — same logic as deployments.js return_to_office 'In Transit'
