...
export default async function handler(req, res) {
    ...
    try {
        ...
        // --- ACTION: GET NOTES (Manual vs System) ---
        if (action === 'get_notes') {
            const { merchant_uuid, type } = req.body;
            let queryReq = supabase.from('merchant_notes').select('*').eq('merchant_id', merchant_uuid);

            if (type === 'manual') {
                queryReq = queryReq.neq('title', 'System Update');
            } else if (type === 'system') {
                queryReq = queryReq.eq('title', 'System Update');
            }

            const { data, error } = await queryReq.order('created_at', { ascending: false });
            if (error) throw error;
            return res.status(200).json({ success: true, data });
        }
        ...
        // --- NEW ACTION: SAVE ATTACHMENT RECORD ---
        if (action === 'save_attachment') {
            const { merchant_uuid, name, description, url } = req.body;
