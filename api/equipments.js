    ...
    try {
        if (action === 'getHydratedData') {
            ...
            const rel = (relData.relations || []).find(r => 
                r.secondObjectKey.includes('equipments') || r.firstObjectKey.includes('equipments') || r.secondObjectKey.includes(EQUIPMENT_SCHEMA)
            );
            if (rel) {
        ...
        if (action === 'createDeployment') {
            ...
            const nId = d.id || d.record?.id;
            if (nId) {
                await fetch(PROXY_URL, { method: "POST", headers: { "Content-Type": "application/json", "Link-Relation": "true" }, body: JSON.stringify({ locationId: LOCATION_ID, associationId: DEPLOY_TO_GEAR_ASSOC, firstRecordId: nId, secondRecordId: gearID }) });
        ...
        if (action === 'createReturn') {
            ...
            const resRet = await fetch(PROXY_URL, { method: "POST", headers: { "Content-Type": "application/json", "Schema-Id": RETURN_SCHEMA }, body: JSON.stringify({ locationId: LOCATION_ID, properties: mappedPayload }) });
            const newRet = await resRet.json();
            const newRetId = newRet.id || newRet.record?.id;
            if (newRetId) {
                await fetch(PROXY_URL, { method: "POST", headers: { "Content-Type": "application/json", "Link-Relation": "true" }, body: JSON.stringify({ locationId: LOCATION_ID, associationId: RETURN_ASSOC_ID, firstRecordId: gearID, secondRecordId: newRetId }) });
        ...
        if (action === 'delete') {
            ...
            await logAction(`Record Deleted: ${id}`);
            return res.status(200).json({ success: true });
        }
